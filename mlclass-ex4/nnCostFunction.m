...
z2=Theta1*a1';
a2 = sigmoid(z2);
h = sigmoid(Theta2*[ones(size(X,1),1)'; a2]);

y_m = zeros(num_labels, size(y,1));
for i=1:size(y,1)
...
for i=1:m
    for k=1:num_labels
        J = J + -y_m(k,i)*log(h(k,i)) - (1-y_m(k,i))*log(1-h(k,i));
    end
end
...
for j=1:size(Theta1,1)
    for k=2:size(Theta1,2)
        regu = regu + Theta1(j,k)^2;
    end
end
for j=1:size(Theta2,1)
    for k=2:size(Theta2,2)
        regu = regu + Theta2(j,k)^2;
    end
end
